...
public enum NMLayerDirection: Int {
    case vertical = 0, horizontal
}


public class NMSegmentUnit: NSObject, NSCopying {
...
public class NMSegmentUnitControlStyle: NSObject {
    public var textFieldBackgroundColor: UIColor = UIColor.clear
    public var textFieldFont: UIFont = kDefaultTextFieldFont
    public var textFieldTextColor: UIColor = UIColor.clear
    public var scrollViewBackgroundColor: UIColor = UIColor.clear
    public var colorOverrides: Dictionary<NMRange<Float>, UIColor>?
}


public protocol NMMultiUnitRulerDataSource {
...
public protocol NMMultiUnitRulerDelegate {
    func valueChanged(measurement: NSMeasurement)
}


public class NMMultiUnitRuler: UIView {
    public var dataSource: NMMultiUnitRulerDataSource? {
        didSet {
            setupViews()
        }
    }
    ...
    private var textViews: Array<NMRangeTextView>?
    public var direction: NMLayerDirection = .horizontal
    ...
    @objc func segmentSelectionChanged(_ sender: UISegmentedControl) {
        ...
        if let dataSource = self.dataSource, let scrollViews = self.scrollViews {
            ...
            if let measurement = self.measurement, let unit = segmentUnit.unit {
                ...
                let minScale = NMRangeMarkerType.minScale(types: segmentUnit.markerTypes)
    ...
    private func setupSegmentViews() -> (Array<UIView>, Array<NMRangeScrollView>, Array<NMRangeTextView>, Array<NMRangePointerView>) {
        var segmentViews: [UIView] = []
        var pointerViews: [NMRangePointerView] = []
        var scrollViews: [NMRangeScrollView] = []
        var textViews: [NMRangeTextView] = []
        if let dataSource = self.dataSource {
    ...
    private func setupPointerView(inSegmentView parent: UIView,
                                  ...
                                  segmentStyle style: NMSegmentUnitControlStyle) -> NMRangePointerView {
        let pointerView = NMRangePointerView(frame: self.bounds)
    ...
    private func setupSegmentScrollView(inSegmentView parent: UIView,
                                        unit segmentUnit: NMSegmentUnit,
                                        segmentStyle style: NMSegmentUnitControlStyle,
                                        range floatRange: NMRange<Float>) -> NMRangeScrollView {
        let scrollView = NMRangeScrollView(frame: self.bounds)
    ...
    private func setupSegmentBottomView(inSegmentView parent: UIView,
        ...
        let textView = NMRangeTextView(frame: self.bounds)
